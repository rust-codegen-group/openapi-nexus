...
This package provides a TypeScript/JavaScript client for the Petstore API API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
{
  "dependencies": {
    "petstore-api": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'petstore-api';
...
```typescript
import { Configuration, Middleware } from 'petstore-api';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'petstore-api';
...
```typescript
import type { User, CreateUserRequest } from 'petstore-api';
