...
This package provides a TypeScript/JavaScript client for the Primitive Array Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install primitive-array-test
```
...
{
  "dependencies": {
    "primitive-array-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'primitive-array-test';
...
```typescript
import { Configuration, Middleware } from 'primitive-array-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'primitive-array-test';
...
```typescript
import type { User, CreateUserRequest } from 'primitive-array-test';
