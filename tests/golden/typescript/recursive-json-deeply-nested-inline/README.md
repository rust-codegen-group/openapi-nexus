...
This package provides a TypeScript/JavaScript client for the Deeply Nested Inline Objects Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install deeply-nested-inline-objects-test
```
...
{
  "dependencies": {
    "deeply-nested-inline-objects-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'deeply-nested-inline-objects-test';
...
```typescript
import { Configuration, Middleware } from 'deeply-nested-inline-objects-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'deeply-nested-inline-objects-test';
...
```typescript
import type { User, CreateUserRequest } from 'deeply-nested-inline-objects-test';
