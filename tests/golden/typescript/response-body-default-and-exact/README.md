...
This package provides a TypeScript/JavaScript client for the Default And Exact Response API API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install default-and-exact-response-api
```
...
{
  "dependencies": {
    "default-and-exact-response-api": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'default-and-exact-response-api';
...
```typescript
import { Configuration, Middleware } from 'default-and-exact-response-api';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'default-and-exact-response-api';
...
```typescript
import type { User, CreateUserRequest } from 'default-and-exact-response-api';
