...
This package provides a TypeScript/JavaScript client for the Response Fallback Test API API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install response-fallback-test-api
```
...
{
  "dependencies": {
    "response-fallback-test-api": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'response-fallback-test-api';
...
```typescript
import { Configuration, Middleware } from 'response-fallback-test-api';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'response-fallback-test-api';
...
```typescript
import type { User, CreateUserRequest } from 'response-fallback-test-api';
