...
This package provides a TypeScript/JavaScript client for the Mixed Property Types Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install mixed-property-types-test
```
...
{
  "dependencies": {
    "mixed-property-types-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'mixed-property-types-test';
...
```typescript
import { Configuration, Middleware } from 'mixed-property-types-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'mixed-property-types-test';
...
```typescript
import type { User, CreateUserRequest } from 'mixed-property-types-test';
