...
This package provides a TypeScript/JavaScript client for the Simple Type Alias Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install simple-type-alias-test
```
...
{
  "dependencies": {
    "simple-type-alias-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'simple-type-alias-test';
...
```typescript
import { Configuration, Middleware } from 'simple-type-alias-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'simple-type-alias-test';
...
```typescript
import type { User, CreateUserRequest } from 'simple-type-alias-test';
