...
This package provides a TypeScript/JavaScript client for the Interface with Enum Reference Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install interface-with-enum-reference-test
```
...
{
  "dependencies": {
    "interface-with-enum-reference-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'interface-with-enum-reference-test';
...
```typescript
import { Configuration, Middleware } from 'interface-with-enum-reference-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'interface-with-enum-reference-test';
...
```typescript
import type { User, CreateUserRequest } from 'interface-with-enum-reference-test';
