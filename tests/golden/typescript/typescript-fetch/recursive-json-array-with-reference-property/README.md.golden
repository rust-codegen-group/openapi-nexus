...
This package provides a TypeScript/JavaScript client for the Array with Reference Property Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install array-with-reference-property-test
```
...
{
  "dependencies": {
    "array-with-reference-property-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'array-with-reference-property-test';
...
```typescript
import { Configuration, Middleware } from 'array-with-reference-property-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'array-with-reference-property-test';
...
```typescript
import type { User, CreateUserRequest } from 'array-with-reference-property-test';
