...
This package provides a TypeScript/JavaScript client for the Complex Union Type Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install complex-union-type-test
```
...
{
  "dependencies": {
    "complex-union-type-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'complex-union-type-test';
...
```typescript
import { Configuration, Middleware } from 'complex-union-type-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'complex-union-type-test';
...
```typescript
import type { User, CreateUserRequest } from 'complex-union-type-test';
