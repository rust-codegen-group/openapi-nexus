...
This package provides a TypeScript/JavaScript client for the Empty Array Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
{
  "dependencies": {
    "empty-array-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'empty-array-test';
...
```typescript
import { Configuration, Middleware } from 'empty-array-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'empty-array-test';
...
```typescript
import type { User, CreateUserRequest } from 'empty-array-test';
