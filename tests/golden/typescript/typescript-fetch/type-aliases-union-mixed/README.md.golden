...
This package provides a TypeScript/JavaScript client for the Union Type with Mixed Members Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install union-type-with-mixed-members-test
```
...
{
  "dependencies": {
    "union-type-with-mixed-members-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'union-type-with-mixed-members-test';
...
```typescript
import { Configuration, Middleware } from 'union-type-with-mixed-members-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'union-type-with-mixed-members-test';
...
```typescript
import type { User, CreateUserRequest } from 'union-type-with-mixed-members-test';
