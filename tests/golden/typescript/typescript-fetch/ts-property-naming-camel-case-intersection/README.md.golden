...
This package provides a TypeScript/JavaScript client for the Intersection Type (allOf) Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install intersection-type-all-of-test
```
...
{
  "dependencies": {
    "intersection-type-all-of-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'intersection-type-all-of-test';
...
```typescript
import { Configuration, Middleware } from 'intersection-type-all-of-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'intersection-type-all-of-test';
...
```typescript
import type { User, CreateUserRequest } from 'intersection-type-all-of-test';
