...
This package provides a TypeScript/JavaScript client for the Duplicate Parameter Names Test API API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install duplicate-parameter-names-test-api
```
...
{
  "dependencies": {
    "duplicate-parameter-names-test-api": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'duplicate-parameter-names-test-api';
...
```typescript
import { Configuration, Middleware } from 'duplicate-parameter-names-test-api';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'duplicate-parameter-names-test-api';
...
```typescript
import type { User, CreateUserRequest } from 'duplicate-parameter-names-test-api';
