...
This package provides a TypeScript/JavaScript client for the Union Type with Inline Objects Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install union-type-with-inline-objects-test
```
...
{
  "dependencies": {
    "union-type-with-inline-objects-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'union-type-with-inline-objects-test';
...
```typescript
import { Configuration, Middleware } from 'union-type-with-inline-objects-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'union-type-with-inline-objects-test';
...
```typescript
import type { User, CreateUserRequest } from 'union-type-with-inline-objects-test';
