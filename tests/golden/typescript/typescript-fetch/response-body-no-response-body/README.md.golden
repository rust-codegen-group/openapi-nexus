...
This package provides a TypeScript/JavaScript client for the No Response Body API API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install no-response-body-api
```
...
{
  "dependencies": {
    "no-response-body-api": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'no-response-body-api';
...
```typescript
import { Configuration, Middleware } from 'no-response-body-api';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'no-response-body-api';
...
```typescript
import type { User, CreateUserRequest } from 'no-response-body-api';
