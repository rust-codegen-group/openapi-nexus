...
This package provides a TypeScript/JavaScript client for the Multi Status API API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
{
  "dependencies": {
    "multi-status-api": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'multi-status-api';
...
```typescript
import { Configuration, Middleware } from 'multi-status-api';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'multi-status-api';
...
```typescript
import type { User, CreateUserRequest } from 'multi-status-api';
