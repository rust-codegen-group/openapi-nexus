...
This package provides a TypeScript/JavaScript client for the Nested Union Type Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install nested-union-type-test
```
...
{
  "dependencies": {
    "nested-union-type-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'nested-union-type-test';
...
```typescript
import { Configuration, Middleware } from 'nested-union-type-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'nested-union-type-test';
...
```typescript
import type { User, CreateUserRequest } from 'nested-union-type-test';
