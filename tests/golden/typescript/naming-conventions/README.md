...
This package provides a TypeScript/JavaScript client for the Naming Conventions Test API. It uses the native [Fetch API](https://fetch.spec.whatwg.org/) for HTTP requests and works in both Node.js and browser environments.
...
```bash
npm install naming-conventions-test
```
...
{
  "dependencies": {
    "naming-conventions-test": "file:../../path/to/generated/package"
  }
}
```
...
```typescript
import { Configuration, DefaultApi } from 'naming-conventions-test';
...
```typescript
import { Configuration, Middleware } from 'naming-conventions-test';
...
```typescript
import { ResponseError, FetchError, RequiredError } from 'naming-conventions-test';
...
```typescript
import type { User, CreateUserRequest } from 'naming-conventions-test';
