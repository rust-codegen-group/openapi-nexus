//! Golden file tests for Rust ureq generator
//!
...
//!   UPDATE_GOLDEN=1 cargo test --test golden_tests_rust_ureq
...
fn golden_dir() -> &'static Path {
    Path::new("tests/golden/rust/rust-ureq")
}

const UPDATE_HINT: &str = "UPDATE_GOLDEN=1 cargo test --test golden_tests_rust_ureq";
    ...
    ($($test_name:ident: $spec_name:expr),* $(,)?) => {
        $(
            ...
            fn $test_name() {
                ...
                let generator = RustUreqCodeGenerator::new(toml::value::Table::new());
                run_golden_test(
                    &generator,
                    golden_dir(),
                    $spec_name,
...
fn test_utoipa_mixed_golden() {
    let generator = RustUreqCodeGenerator::new(utoipa_config());
    run_golden_test(
        &generator,
        golden_dir(),
        "utoipa-mixed",
...
fn test_utoipa_with_extra_derives_golden() {
    let generator = RustUreqCodeGenerator::new(utoipa_with_extra_derives_config());
    run_golden_test(
        &generator,
        golden_dir(),
