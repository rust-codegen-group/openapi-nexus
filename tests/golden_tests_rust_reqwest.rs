//! Golden file tests for Rust reqwest generator
//!
...
fn golden_dir() -> &'static Path {
    Path::new("tests/golden/rust/rust-reqwest")
}

const UPDATE_HINT: &str = "UPDATE_GOLDEN=1 cargo test --test golden_tests_rust_reqwest";
    ...
    ($($test_name:ident: $spec_name:expr),* $(,)?) => {
        $(
            ...
            fn $test_name() {
                ...
                let generator = RustReqwestCodeGenerator::new(toml::value::Table::new());
                run_golden_test(
                    &generator,
                    golden_dir(),
                    $spec_name,
