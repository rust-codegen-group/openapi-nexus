//! Golden file tests for Python httpx generator
//!
...
//!   UPDATE_GOLDEN=1 cargo test --test golden_tests_python_httpx
...
fn golden_dir() -> &'static Path {
    Path::new("tests/golden/python/python-httpx")
}

const UPDATE_HINT: &str = "UPDATE_GOLDEN=1 cargo test --test golden_tests_python_httpx";
    ...
    ($($test_name:ident: $spec_name:expr),* $(,)?) => {
        $(
            ...
            fn $test_name() {
                ...
                let generator = PythonHttpxCodeGenerator::new(toml::value::Table::new());
                run_golden_test(
                    &generator,
                    golden_dir(),
                    $spec_name,
