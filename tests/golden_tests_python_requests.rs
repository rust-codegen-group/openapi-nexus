//! Golden file tests for Python requests generator
//!
//! These tests compare generated Python code against known-good golden files.
//! To update golden files after intentional changes, run:
//!   UPDATE_GOLDEN=1 cargo test --test golden_tests_python_requests
...
fn golden_dir() -> &'static Path {
    Path::new("tests/golden/python/python-requests")
}

const UPDATE_HINT: &str = "UPDATE_GOLDEN=1 cargo test --test golden_tests_python_requests";
    ...
    ($($test_name:ident: $spec_name:expr),* $(,)?) => {
        $(
            ...
            fn $test_name() {
                ...
                let generator = PythonRequestsCodeGenerator::new(toml::value::Table::new());
                run_golden_test(
                    &generator,
                    golden_dir(),
                    $spec_name,
