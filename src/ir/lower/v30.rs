...
use crate::spec::oas30::spec::{
    self as oas, AdditionalProperties, ObjectOrReference, ObjectSchema, Schema,
};
...
pub fn lower_v30(spec: &oas::OpenApiV30Spec) -> Result<IrSpec, LowerError> {
    ...
    // 2. Lower operations (paths is required in OAS 3.0, not Option)
    ...
    // (No webhooks in OAS 3.0)

    // 3. Lower security schemes
...
fn server_path_prefix(servers: &[crate::spec::oas30::spec::Server]) -> String {
    servers
        .first()
        .map(|s| {
            let u = &s.url;
...
fn strip_server_path_prefix(path: &str, servers: &[crate::spec::oas30::spec::Server]) -> String {
...
impl<'a> LowerCtx<'a> {
    fn new(spec: &'a oas::OpenApiV30Spec) -> Self {
    ...
    fn try_lower_tagged_union(
        ...
        let patterns: Vec<Option<TaggedEnumPattern>> = obj
            .one_of
            .iter()
            .map(TaggedEnumPattern::detect_from_schema_v30)
            .collect();
    ...
    fn lower_object(
        ...
        let additional_properties = match &obj.additional_properties {
            Some(AdditionalProperties::Boolean(b)) => {
                if *b {
                    Some(IrTypeExpr::Any)
                } else {
                    None
            ...
            Some(AdditionalProperties::Schema(boxed_schema)) => {
                Some(self.lower_schema_value(boxed_schema)?)
            }
            None => None,
        };
    ...
    /// Lower a `Schema` value (Boolean or Object) to an IrTypeExpr.
    fn lower_schema_value(&mut self, schema: &Schema) -> Result<IrTypeExpr, LowerError> {
        match schema {
            Schema::Boolean(_) => Ok(IrTypeExpr::Any),
            Schema::Object(boxed_ref) => self.lower_schema_ref(boxed_ref),
        }
    }
    ...
    fn lower_type_from_schema(&self, obj: &ObjectSchema) -> IrTypeExpr {
        ...
        let base = match obj.schema_type.as_deref() {
            Some(type_str) => type_string_to_type_expr(type_str, obj),
            None => {
        ...
        // OAS 3.0 nullable: wrap in Nullable if nullable: true
        if obj.nullable == Some(true) {
    ...
    fn lower_parameter(&mut self, param: &oas::Parameter) -> Result<IrParameter, LowerError> {
        ...
        Ok(IrParameter {
            ...
            location: match param.location {
                ...
                oas::ParameterIn::Body => ParameterLocation::Query,
                oas::ParameterIn::FormData => ParameterLocation::Query,
            },
            type_expr,
...
/// OAS 3.0: schema type is a single string, not a typed enum.
fn type_string_to_type_expr(type_str: &str, obj: &ObjectSchema) -> IrTypeExpr {
    match type_str {
        "string" => match obj.format.as_deref() {
                ...
                match items.as_ref() {
                    Schema::Object(boxed_ref) => match boxed_ref.as_ref() {
                        ...
                        ObjectOrReference::Object(inner) => {
                            let inner_type = type_string_to_type_expr(
                                inner.schema_type.as_deref().unwrap_or("string"),
                                inner,
                            );
        ...
        "object" => {
            if let Some(ap) = &obj.additional_properties {
                match ap {
                    ...
                    AdditionalProperties::Schema(boxed_schema) => match boxed_schema.as_ref() {
                        Schema::Boolean(_) => IrTypeExpr::Map(Box::new(IrTypeExpr::Any)),
                        Schema::Object(boxed_ref) => match boxed_ref.as_ref() {
...
fn is_array_with_promotable_items(obj: &ObjectSchema) -> bool {
    let is_array = obj.schema_type.as_deref() == Some("array");
    if !is_array {
        return false;
    }
    if let Some(items) = &obj.items
...
fn lower_info(info: &oas::Info) -> IrInfo {
    IrInfo {
        ...
        license: info.license.as_ref().map(|l| IrLicense {
            ...
            identifier: None, // OAS 3.0 has no license identifier
        }),
    }
}
...
mod tests {
    ...
    fn lower_yaml(yaml: &str) -> IrSpec {
        ...
        lower_v30(parsed.as_v30().unwrap()).unwrap()
    }
